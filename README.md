# Image Crop and Resize Tool

A lightweight web-based tool for cropping and resizing images with precision. It provides an intuitive interface to select specific areas, fine-tune dimensions, and preview changes in real time. Quickly export optimized images for seamless use in web, UI, and design workflows.

Launch Now: [https://deshan.lk/apps/jwt-generator](https://deshan.lk/apps/jwt-generator)

## Key Features

- Precisely crop images with adjustable selection areas.
- Resize images by setting custom width and height values.
- Real-time preview for accurate editing before export.
- Download optimized images instantly for web and design use.

## Libraries and Frameworks Used

- [Vue.js](https://vuejs.org/) - Progressive framework for building user interfaces

## Assets and Resources Used

- [Inter](https://fonts.google.com/specimen/Inter) font by Rasmus Andersson

### Developed by Deshan Nawanjana

[Deshan.lk](https://deshan.lk/)
&ensp;|&ensp;
[LinkedIn](https://www.linkedin.com/in/deshan-nawanjana/)
&ensp;|&ensp;
[Facebook](https://www.facebook.com/mr.dnjs)
&ensp;|&ensp;
[GitHub](https://github.com/deshan-nawanjana)
&ensp;|&ensp;
[YouTube](https://www.youtube.com/@deshan-nawanjana)
&ensp;|&ensp;
[X](https://x.com/DeshanNawanjana)
&ensp;|&ensp;
[Instagram](https://www.instagram.com/_d.boy_/)
&ensp;|&ensp;
[Reddit](https://www.reddit.com/user/DeshanNawanjana/)